...
public class PropertiesLogger implements ApplicationListener<ApplicationEnvironmentPreparedEvent> {
    ...
    public void onApplicationEvent(@NonNull ApplicationEnvironmentPreparedEvent event) {
        ...
        log.atDebug().log(()-> STR."Logging \{propertySelectorType.forLogging()}");
        ...
        props.insert(0, STR."""
                ...
                \{this.propertySourceNames.stream().reduce("",(l, e)-> STR."\{l}\{lineSeparator()}- \{e}")}
                ===============================================================================================
                """);
    ...
    private Optional<PropertySelectorEnum> getSelectorFromProperty(String property) {
        if(property!=null){
            try{
                ...
                log.atTrace().log(()-> STR."Impossible de convertir \{property} en une constante de PropertySelectorEnum. Le PropertySelector par défaut sera utilisé.");
            }
        }
        return Optional.empty();
    }

    private boolean filterFromPropertySelector(@NonNull String property) {
        if (! this.propertySelector.filter(property)){
            log.atDebug().log(()->STR."\{property} ne commence pas par un des prefix retenus pour être loguée");
            return false;
        }
        return true;
    }
    ...
    private boolean sourceWillBeProcessed(PropertySource<?> propertySource) {
        ...
        if (ignoredPropertySources.contains(propertySource.getName())){
            log.atDebug().log(()->STR."\{propertySource} sera ignorée");
            return false;
        }
        return true;
    }
    ...
    private boolean isEnumerable(PropertySource<?> propertySource) {
        if (! (propertySource instanceof EnumerablePropertySource)){
            log.atDebug().log(()->STR."\{propertySource} n'est pas EnumerablePropertySource : impossible à lister");
            return false;
        }
        return true;
    }
    ...
    public enum PropertySelectorEnum {
        ALL(s ->true, ()->"all properties"),
        NONE(s ->false, ()->"no properties"),
        PREFIX(key->prefixForSelectedProps.stream().anyMatch(key::startsWith), () -> STR."properties starting with \{prefixForSelectedProps}");
